...
explainer like this one, checked by the validator. This repo is still private; its visibility lines are updated at the next gate (S1), right
before its own audited flip.
...
./scripts/validate-plugins.sh          # the two new explainer checks: PASS
